...
def test_invent_parses_and_filters_effects(monkeypatch):
    parsed = ai_recipe.AIRecipe(
        reasoning="Перца не жалели — жжёт нутро и злит.",
    ...
    name, lore, reasoning, proposal = got
    assert name == "Огневая солянка «У плахи»" and lore.startswith("Наперчено")
    assert reasoning.startswith("Перца")                           # «Повар рассудил» протянут
...
def test_refusal_falls_back(monkeypatch):
    parsed = ai_recipe.AIRecipe(reasoning="r", name="x", lore="y", effects={"dmg": 1})
...
def test_text_moderation_truncates_and_strips(monkeypatch):
    ...
    _patch(monkeypatch, _Client(resp=_Resp(parsed)))
    name, lore, reasoning, _ = _run(ai_recipe.invent(["game", "milk"], 8))
    assert len(name) <= ai_recipe.NAME_MAX and "\n" not in name and "\t" not in name
    assert len(lore) <= ai_recipe.LORE_MAX
    assert len(reasoning) <= ai_recipe.REASONING_MAX
...
def test_end_to_end_build_uses_ai_flavor_but_code_numbers(monkeypatch):
    ...
    parsed = ai_recipe.AIRecipe(reasoning="Дичь да травы — сила и лёгкость.",
                                name="Царское жаркое", lore="Пир на весь Недоливск.",
                                effects={"dmg": 99, "crit": 99})   # ИИ просит имбу
    _patch(monkeypatch, _Client(resp=_Resp(parsed)))
    ing = ["game", "herbs", "honey"]
    ai = _run(ai_recipe.invent(ing, recipes.recipe_budget(ing)))
    name, lore, reasoning, proposal = ai
    rec = recipes.build_recipe(ing, ai_name=name, ai_lore=lore,
                               ai_reasoning=reasoning, ai_proposal=proposal)
    assert rec["name"] == "Царское жаркое"                          # флейвор — ИИ
    assert rec["reasoning"] == "Дичь да травы — сила и лёгкость."   # «Повар рассудил» в рецепте
