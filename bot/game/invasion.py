...
# ═══ БОЕВОЙ ЗАПУСК ОРДЫ — ТРИ согласованных флага (проверяется тестом) ═══════════
# ⚙️ ОТКРЫТО ВСЕМ (боевой запуск): участие + карта + награды. Спавн орды — по-прежнему
# ВРУЧНУЮ админом (/orc), авто выключено. Откатить в обкатку = все три флага назад:
#   обкатка: TEST_MODE=True,  REWARDS_ENABLED=False, MAP_PUBLIC=False.
# ⚠️ Флаги должны быть согласованы (test_launch_flags_consistent): нельзя открыть
# участие (TEST_MODE=False), не открыв карту и награды — иначе игроки тратят ресурсы
# на приготовления в невидимую орду без наград.

# Участие (запись «в строй»/панель/приготовления) — ОТКРЫТО всем; /orc шлёт анонсы в чаты.
TEST_MODE = False
# Награды за победу и штрафы за поражение — НАЧИСЛЯЮТСЯ.
REWARDS_ENABLED = True
# Орда на карте (/world/invasion) и сводка итогов (/api/invasion/result) — видны ВСЕМ.
MAP_PUBLIC = True
