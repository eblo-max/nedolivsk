...
WONDERS: dict[str, WonderDef] = {
    "wall": WonderDef(
        ...
        phases=(
            # Калибровка Ф5 (05.07.2026): база ÷6 от исходной (1200/1800/2400) под
            # реальную популяцию прода (66 активных, медиана 324 золота, ур.1-2).
            # Цель = base×активные; при base 200/300/400 Твердыня ≈10-14 дней явки
            # (Монте-Карло на настоящем wonder.py). Само-масштабируется по миру.
            Phase("foundation", "Ров и фундамент", 200),
            Phase("walls",      "Стены и ворота",  300),
            # Финал — грандиозный: активное комьюнити прошло ф1+ф2 за часы (06.07.2026),
            # так что «Башни и детинец» = главный многодневный рывок (base 400→1000).
            Phase("keep",       "Башни и детинец", 1000),
        ),
...
PHASE_BONUS_ZODAR = 10      # базовый бонус зодаров за закрытую фазу (× множитель брекета).
#                             Калибровка Ф5: 6→10 — в бедном мире бонус-за-участие
#                             (а не за богатство) справедливо доминирует над вкладом.
