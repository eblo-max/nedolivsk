...
# поэтому пороги задраны против по-чатовых (×2) — глубокая ликвидность держит
# цену стабильнее (урок WoW: общий рынок = меньше скачков). При росте числа
# активных чатов эти пороги имеет смысл поднимать дальше.
MARKET_SATURATION = 320     # завал, при котором цена падает до пола
MARKET_PRICE_FLOOR = 0.55   # минимум множителя fv при заваленном рынке
MARKET_SHORTAGE = 240       # дефицит (−glut), при котором цена на потолке
