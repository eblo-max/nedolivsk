...
# Динамический рынок: оптовая цена (fv) проседает от завала и восстанавливается.
# Завал (glut) — единицы товара, выброшенные на рынок чата; тает экспоненциально.
...
MARKET_ABSORB_HOURS = 14    # постоянная времени впитывания излишков (τ)


def market_factor(glut: float) -> float:
    """Множитель справедливой цены от завала рынка: 1.0 (пусто) → пол (завал)."""
    if glut <= 0:
        return 1.0
