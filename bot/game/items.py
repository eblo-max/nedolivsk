...
GEAR_COST_MULT = 1.5
GEAR_RES_MULT = 2.5
# Пол расхода ОБЫЧНОГО сырья на ★ (базовый ярус): не меньше этого каждого ресурса.
# Ярус множит сверху (★ ≥250 → ★★ ×3 ≥750 → ★★★ ×8 ≥2000) — «больше на 2, ещё больше на 3».
GEAR_RES_FLOOR = 250
...
def tier_cost(item: "Item", tier: int) -> dict:
    ...
    for k, v in item.cost.items():
        ...
        else:                                     # обычное сырьё: пол на ★, ярус сверху
            base1 = max(GEAR_RES_FLOOR, math.ceil(v * GEAR_RES_MULT))
            out[k] = base1 * tmult
    return out
