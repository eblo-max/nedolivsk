...
from bot.game import balance, buff, market, story_state, trade, worldevent
...
def _sell(player: Player, offer: dict, unit: int) -> tuple[int, int]:
    ...
    if qty <= 0:
        return 0, 0
    gold = int(qty * unit * buff.sale_mult(player)
               * worldevent.sale_mult(player))  # баф + погода (Лихорадка/Буря/Засуха)
...
async def _finish_sale(callback: CallbackQuery, player: Player, offer: dict,
                       unit: int, kind: str, session: AsyncSession, world=None) -> None:
...
async def cb_trade(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    if arg == "ok":  # согласие на контр-цену
        ...
        await _finish_sale(callback, player, offer, unit, kind, session, world)
        return
