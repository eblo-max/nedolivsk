...
def fair_pre_announce(minutes_left: int) -> str:
    ...
    return (
        "📯 <b>Гонец на площади!</b>\n\n"
        f"Через {_fmt_minutes(minutes_left)} в Недоливске грянет "
        f"<b>ЯРМАРКА</b>. Купцы съезжаются, гости при деньгах — спрос на "
        f"выпивку и еду взлетит <b>×{balance.FAIR_DEMAND_MULT:g}</b>.\n\n"
        "Гоните бригады за сырьём, набивайте погреба бочками и блюдами — "
        "на ярмарке всё разлетится влёт. Кто не запасся, будет кусать локти."
    )
...
def fair_open_announce() -> str:
    ...
    return (
        "🎪 <b>ЯРМАРКА ОТКРЫТА!</b>\n\n"
        f"Площадь гудит, гости валят валом — спрос на выпивку и еду "
        f"<b>×{balance.FAIR_DEMAND_MULT:g}</b> на {balance.FAIR_DURATION_HOURS} ч!\n\n"
        "Жми «гг», собирай доход и сбывай товар, пока купцы при золоте. "
        "Лучшего дня для торговли не будет."
    )
...
def fair_close_announce() -> str:
    ...
    return (
        "🌙 <b>Ярмарка свернулась.</b>\n\n"
        "Купцы разъехались, площадь опустела до завтра. Кто успел сбыть "
        "бочки — нынче при золоте; остальные точат бочки к следующей."
    )
