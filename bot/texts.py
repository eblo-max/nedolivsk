...
WELCOME = (
    ...
    "<blockquote>Захолустный городишко, где посуду моют раз в год по обещанию, "
    "а за лишний вопрос живо пересчитают зубы.</blockquote>\n\n"
    "Тут ты заводишь свой трактир и тащишь его в богатейший двор округи: "
    "гонишь работяг за добром, осваиваешь ремёсла, торгуешься до хрипоты "
    "и гребёшь золото лопатой.\n\n"
    "И это не игрушка по таймеру. Тут <b>живой город</b>: жители со своими "
    "хитрыми рожами, заговоры, фракции, склоки — и каждый твой косяк город "
    "запомнит да при случае припомнит.\n\n"
    "Захочешь развернуться по-настоящему — волоки бота в общий чат. У всей "
    "вашей компании заведётся целый Недоливск, один на всех.\n\n"
    "📣 Новости, обновы и весь движ — в канале "
    "<a href=\"https://t.me/nedolivsk\">@nedolivsk</a>. Подпишись, чтоб не проморгать.\n\n"
    "Ну хорош пялиться. За дело."
)

LIVING_CITY = (
    "🏰 <b>ЖИВОЙ НЕДОЛИВСК</b>\n"
    "Заруби на носу: это не кликалка по таймеру. Городишко живёт "
    "своей склочной жизнью, и ему плевать, удобно тебе или нет.\n\n"
    "🍻 <b>Гости с историями.</b> Только присел барыш считать — на порог уже "
    "припёрся какой-нибудь прохиндей: захмелевший рыцарь канючит в долг, стражник тянет "
    "на лапу, бард лезет с дурацкими куплетами, ведьма впаривает гадание, картёжник "
    ...
    "гуляк с золотом. У каждой рожи своя память и своя многоходовка, что "
    ...
    "🎪 <b>И расхлёбывают все.</b> Поднимется кто-то один — в городе аукается "
    ...
    "Короче: не один ты в песочнице. Ты житель живого склочного городка."
)

ADD_TO_CHAT = (
    "👥 <b>Недоливск на всю компанию</b>\n"
    "Весь сок — в общем чате. Затащи бота в свою беседу, и заведётся "
    "<b>общий город, один на всех</b>. В одиночку скучать — последнее дело.\n\n"
    "Чё наваришь:\n"
    "🏘 <b>Один Недоливск на чат</b> — вы всей толпой качаете одни фракции и "
    "общую судьбу. Один начудил — расхлёбывают все.\n"
...
# Ответ на «гг карта»: карта живёт в мини-аппе — даём кнопку/ссылку на неё.
MAP_HINT = (
    "🗺 <b>Карта мира Недоливска</b>\n"
    "Таверны игроков, континенты, короны лидеров и события мира — на живой "
    "интерактивной карте. Открой её ниже 👇"
)
...
def fair_pre_announce(minutes_left: int) -> str:
    ...
    return (
        ...
        "<i>Купчина ждать не станет — ему дела нет до твоей лени-матушки.</i>"
    )
...
def fair_open_announce() -> str:
    ...
    return (
        ...
        "в дрова окончательно. Бочки сами себя не продадут — шевели "
...
def fair_close_announce() -> str:
    ...
    return (
        "🌙 <b>Всё, приехали, лавочка закрыта.</b>\n\n"
...
def tavern_screen(player: Player, tavern: Tavern) -> str:
    ...
    region = wmap.continent_name(player.region, player.id)   # локация = континент своей зоны
...
def income_empty() -> str:
    return ("💤 Касса пуста, как башка завсегдатая — пассив ещё капает.\n"
            "Пока копится, не сиди сиднем: отправь бригаду за ресурсами или сходи "
            "на охоту.")
...
_RAID_FLAVOR: dict[str, tuple[str, str]] = {
    ...
    "demon_slime": (
        "Из преисподней выперло — потянуло серой.",
        "Всем кабаком на тварь, пока не спалила полгорода и не наплодила бесов. "
        "Кто отсиделся по углам — тот и без доли.",
    ),
    "dragon": (
...
def raid_fight_ping() -> str:
    return ("⚔️ <b>ОН ЗДЕСЬ — босс дошёл, пора БИТЬ!</b>\n"
            "Открой кабак (/start) и жми <b>«⚔️ РЕЙД-БОСС — В БОЙ!»</b> в меню — "
            "лупить можно <b>прямо из лички</b>, даже если сообщение в чате удалили.\n"
            "Кто в деле — тот и в доле: золото меж бойцов, а кому-то с туши падёт "
            "легендарный трофей. Не отсиживайся!")
...
# Знатные гости (Ф2): славная таверна манит богатую публику — заходят и отсыпают
# чаевые. (эмодзи, имя-титул). Выбор — по seed сделки (детерминизм в тестах).
NOBLE_GUESTS: tuple[tuple[str, str], ...] = (
    ("🎩", "боярин Твердислав"), ("👑", "воевода Ратибор"),
    ("💰", "купец-гость Аникей"), ("📜", "староста Пров"),
    ("⛪", "игумен Феофан"), ("🗝", "ключница Малуша"),
    ("🐴", "заезжий барон"), ("🍷", "винный откупщик Сила"),
)


def retail_noble(name_i: int, tip: int) -> dict:
    """Данные о знатном госте для мини-аппа: кто зашёл и сколько чаевых отсыпал."""
    emoji, name = NOBLE_GUESTS[name_i % len(NOBLE_GUESTS)]
    return {"emoji": emoji, "name": name, "tip": int(tip)}


def retail_noble_line(name_i: int, tip: int) -> str:
    """Строка для текстового бота/ленты: «🎩 Заглянул боярин … — чаевые +N»."""
    emoji, name = NOBLE_GUESTS[name_i % len(NOBLE_GUESTS)]
    return f"{emoji} Заглянул(а) {name} — чаевые <b>+{int(tip)}</b> 🪙!"
...
def invasion_gather_screen(inv) -> str:
    """Анонс сбора — статичный призыв. Живой отсчёт, состав и сбор — на КАРТЕ
    (плашка регистрации), поэтому в чате счётчик не дёргаем каждую минуту."""
    from bot.game import invasion as invmod
    lines = [
        "🪓 <b>ОРДА ОРКОВ ИДЁТ НА НЕДОЛИВСК!</b>",
        "<i>С северных снегов валит орава зеленомордых — учуяли наш эль да набитые "
        "погреба. Жгут, жрут, тащат всё, что не приколочено. Не встретим всем миром — "
        "к утру от кабаков одни головешки.</i>",
    ]
    e = invmod.escal_of(inv)
    if e > 1.001:                          # орда усилена прошлыми победами — показываем
        lines.append(
            f"<i>⚔ Эта орда злее прежних: после ваших побед зеленомордые пришли "
            f"<b>×{e:.2f}</b> к живучести и удару. Лёгкой драки не ждите.</i>")
    lines += [
        "",
        "Жми «⚔️ Поднять войско» — таверна вышлет дружину, роль определит снаряга "
        "(🛡 броня → авангард, ⚔️ урон → рубаки, 🔭 удача → разведка). Числом не "
        "подкачай — мало придём, затопчут. Кто бился — тому и добыча.",
        "",
        "<i>Отсчёт, состав войск и сама битва — на карте. 🗺</i>",
    ]
    return "\n".join(lines)
...
def invasion_push_dm(inv) -> str:
    ...
    return ("🪓 <b>Орда орков прёт на Недоливск!</b>\n"
            "<i>Зеленомордые с севера учуяли твой эль и погреба.</i> Открой кабак и "
            "подними войско — всем миром отобьёмся, добыча по вкладу. Сбор недолгий, "
            "не проспи!")
...
def mill_back_dm(grain: int) -> str:
    """Пуш в личку: телега с мельницы вернулась, зерно ждёт сбора на карте."""
    return ("🛒🌾 <b>Телега вернулась с мельницы!</b>\n"
            f"Привезла зерна — <b>+{grain}</b> ждёт на карте. Загляни и забери, "
            "пока крысы из погреба не пронюхали.")
