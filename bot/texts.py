...
def invasion_gather_screen(inv) -> str:
    ...
    return "\n".join([
        ...
        "",
        f"🏰 Под ружьём: <b>{n}</b> таверн — {_inv_roles(inv)}",
        f"⏳ Войско выступает через <b>{left // 60}:{left % 60:02d}</b>",
        "",
        "Жми «⚔️ Поднять войско» — таверна вышлет дружину, роль определит снаряга "
        "(🛡 броня → авангард, ⚔️ урон → рубаки, 🔭 удача → разведка). Нужен "
        "<b>микс</b> и числом не подкачать — мало придём, затопчут. Кто бился — "
        "тому и добыча.",
    ])
